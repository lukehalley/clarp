      ...
      try {
        ...
        if (!res.ok && res.status === 404) {
          ...
          if (data.projects && data.projects.length > 0) {
            ...
            // Redirect if not a person
            if (entity.entityType && entity.entityType !== 'person') {
              const route = entity.entityType === 'organization' ? 'org' : 'project';
        ...
        console.error('[PersonPage] Failed to fetch:', err);
        setProject(null);
      } finally {
        setIsLoading(false);
      }
